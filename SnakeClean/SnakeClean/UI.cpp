...
namespace SnakeGame
{
    void InitUI(UI& ui)
    {
        ...
        // MAIN MENU
        ui.menuTitle.setFont(ui.font);
        ui.menuTitle.setString("SNAKE GAME");
        ui.menuTitle.setCharacterSize(48);
        ui.menuTitle.setFillColor(sf::Color(0, 255, 100));
        sf::FloatRect titleBounds = ui.menuTitle.getLocalBounds();
        ui.menuTitle.setOrigin(titleBounds.width / 2.f, titleBounds.height / 2.f);
        ui.menuTitle.setPosition(SCREEN_WIDTH / 2.f, MENU_TITLE_Y);
    ...
    void DrawMainMenu(UI& ui, sf::RenderWindow& window, int selectedItem, const sf::Sprite& menuBg)
    {
        window.clear();
        window.draw(menuBg);
        ...
        {
            ui.menuItems[i].setFillColor(i == selectedItem ? sf::Color(0, 255, 100) : sf::Color(255, 182, 193));
            window.draw(ui.menuItems[i]);
        }
        window.display();
    }
